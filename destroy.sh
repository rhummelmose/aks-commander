...
# Ensure portability
destroy_sh_script_path="$( cd "$( dirname "${BASH_SOURCE[0]}" )" >/dev/null 2>&1 && pwd )"

# Terraform init
source "${destroy_sh_script_path}/terraform/shared/init.sh" $terraform_module

terraform destroy -auto-approve -var-file="${destroy_sh_script_path}/terraform_${terraform_module}.tfvars" "${destroy_sh_script_path}/terraform/${terraform_module}"
